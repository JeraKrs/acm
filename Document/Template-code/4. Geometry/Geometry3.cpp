...
using namespace Ball {
	double getBallLength (double lat1, double lot1, double lat2, double lot2) {
...
bool balance (Point3 p) {
	...
	for (int i = 0; i < n; i++) {
		for (int j = i+1; j < n; j++) {
			...
			int k; for (k = 0; k < n; k++) if (k!=i&&k!=j) break;
			Point3 pk = flat[k];
			bool plus = false, minus = false;
			Point3 nor = getCross(getCross(pj-pi, pk-pi), pj-pi);
			for (int x = 0; x < n; x++) if (x!=i&&x!=j) {
				double temp = getDot(nor, flat[x]-pi);
				if (dcmp(temp) > 0)
					plus = true;
				else if (dcmp(temp) < 0)
					minus = true;
			...
			if (dcmp(getDistancePointToLine(p, pi, pj) - 0.2) < 0)
				return false;
		}
	}
	return true;
...
/*
   Point3 shadow(Point3 p, Point3 a, Point3 b, Point3 c) {
   Point3 u = getCross(b-a, c-a);
   Point3 v = getCross(b-a, c-a);
   }
   */
		...
		for (int j = i+1; j < 5; j++) {
			for (int k = j+1; k < 5; k++) {
				if (sameSide(P[i], P[j], P[k])) {
					Point3 g = getPlaneProjection (G, P[i],  getNormal(P[i], P[j], P[k]));

					if (balance(g)) {
						double dis = getDistancePointToPlane(F, P[i], getNormal(P[i], P[j], P[k]));
						ansMax = max(ansMax, dis);
